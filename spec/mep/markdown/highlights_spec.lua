...
describe('mep.markdown.highlights', function()
  ...
  describe('define_frontmatter', function()
    it('links MepMarkdownFrontmatter to ColorColumn', function()
      clear('MepMarkdownFrontmatter')
    ...
    it('does not overwrite a group a user already customized (default = true)', function()
      clear('MepMarkdownFrontmatter')
      vim.api.nvim_set_hl(0, 'MepMarkdownFrontmatter', { bg = '#123456' })
      highlights.define_frontmatter()
      local hl = vim.api.nvim_get_hl(0, { name = 'MepMarkdownFrontmatter' })
      assert.are.equal('#123456', string.format('#%06x', hl.bg))
    end)
  end)
end)

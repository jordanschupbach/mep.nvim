...
describe('mep.git.gutter', function()
  ...
  -- Respond to the most recently spawned job as if `git show :relpath`
  -- succeeded with `content` (a list of lines) as the indexed blob.
  local function respond_indexed(content, code)
    local job = last_job()
    if #content > 0 then
  ...
  describe('attach', function()
    ...
    it('spawns `git show :relpath` for a file inside a git repo', function()
  ...
  describe('recompute / get_hunks / signs', function()
    it('populates hunks once the indexed content resolves', function()
      local bufnr = make_repo_file({ 'a', 'B', 'c' })
      gutter.attach(bufnr)
      respond_indexed({ 'a', 'b', 'c' })
    ...
    it('treats an untracked file (git show fails) as fully added', function()
      ...
      gutter.attach(bufnr)
      respond_indexed({}, 1) -- git show exits non-zero
      local hunks = gutter.get_hunks(bufnr)
      assert.are.equal(1, #hunks)
  ...
  describe('stage_hunk', function()
    ...
    it('recomputes on a successful apply', function()
      ...
      assert.are.same({ 'git', 'show', ':foo.lua' }, last_job().cmd)
    end)

...
describe('mep.picker.config', function()
  ...
  it('has sensible defaults', function()
    ...
    assert.are.same({}, config.defaults.triggers.buffer_search)
  end)
  ...
  it('overrides a trigger, leaving sibling triggers at their default', function()
    local opts = config.setup({ triggers = { buffer_search = { '/' } } })
    assert.are.same({ '/' }, opts.triggers.buffer_search)
  end)
end)

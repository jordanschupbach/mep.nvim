...
- [x] "Poly mode" — real syntax highlighting *and* real LSP features (hover/definition/references/rename/diagnostics/manual completion) for each language a `#+begin_src` block uses, active by default. Highlighting is tree-sitter language injection (`queries/org/injections.scm`, no custom predicates — see its own header comment for why); LSP is `mep.org.polyglot`'s otter.nvim-style shadow-buffer bridge (one hidden real buffer per language, kept line-for-line in sync with that language's block bodies so no position translation is ever needed, letting whatever server `mep.lsp`/`vim.lsp.enable` already attaches for that filetype answer requests as if it were editing a real file in that language). See the README's `mep.org` section ("Poly mode") for the full writeup.
